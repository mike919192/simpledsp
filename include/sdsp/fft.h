...
public:
    constexpr static double Value0()
    {
        return 0.0;
    }
    constexpr static double Value90()
    {
        return 1.0;
    }
    ...
    constexpr static double Sym0()
    {
        return -1.0;
    }
    constexpr static double Sym90()
    {
        return 1.0;
    }
};
        ...
        for (uint j{ 0 }; j < N; j += (i_group_size << 2)) {
            for (uint k{ 0 }; k < i_group_size; k++) {
                ...
                std::complex<double> temp1{ coeff_index1 > 0 ?
                                                data.at(index1) * wCoeffs.at(coeff_subscript).at(coeff_index1) :
                                                data.at(index1) };
                std::complex<double> temp2{ coeff_index2 > 0 ?
                                                data.at(index2) * wCoeffs.at(coeff_subscript).at(coeff_index2) :
                                                data.at(index2) };
                std::complex<double> temp3{ coeff_index3 > 0 ?
                                                data.at(index3) * wCoeffs.at(coeff_subscript).at(coeff_index3) :
                                                data.at(index3) };
                std::complex<double> temp4{ coeff_index4 > 0 ?
                                                data.at(index4) * wCoeffs.at(coeff_subscript).at(coeff_index4) :
                                                data.at(index4) };
